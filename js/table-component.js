class TableComponent {
    ...
    createActionButton(action, item) {
        ...
        buttonHtml = `<button ${actionTypeAttribute} class="${action.class || ''}" type="button" ${combinedAttributes}>${action.icon ? `<i class="${action.icon}"></i> ` : ''}${action.label || ''}</button>`;
        ...
        data.forEach(item => {
            ...
            if (visibleColumns.some(c => c.field === 'actions')) {
                ...
                if (this.options.enableDelete && this.options.deleteRoute && !this.options.customActions.some(a => a.label === 'Delete')) {
                    actionsHtml += this.createActionButton({
                        type: 'modal',
                        class: 'bg-red-600 hover:bg-red-700 text-white py-1 px-2 rounded text-xs focus:outline-none transition-colors duration-200',
                        icon: 'fa fa-trash',
                        label: 'Delete',
    ...
    updateCardView(data) {
        ...
        data.forEach(item => {
            ...
            if (visibleColumns.some(c => c.field === 'actions')) {
                let hasDefaultEdit = false;
                let hasDefaultDelete = false;
                actionsHtml += `<div class="flex justify-end space-x-1 mt-3 pt-3 border-t border-gray-200">`;

                if (this.options.editable && !this.options.customActions.some(a => a.label === 'Edit')) {
                    actionsHtml += this.createActionButton({ type: 'button', class: 'bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded text-xs', icon: 'fa fa-edit', label: 'Edit', modalId: 'edit-modal-container', route: this.options.editRoute }, item);
                    hasDefaultEdit = true;
                }
                if (this.options.enableDelete && this.options.deleteRoute && !this.options.customActions.some(a => a.label === 'Delete')) {
                    actionsHtml += this.createActionButton({ type: 'modal', class: 'bg-red-600 hover:bg-red-700 text-white py-1 px-2 rounded text-xs focus:outline-none transition-colors duration-200', icon: 'fa fa-trash', label: 'Delete', modalId: 'delete-modal', method: 'DELETE', route: this.options.deleteRoute }, item);
                    hasDefaultDelete = true;
                }
                this.options.customActions.forEach(action => {
                    if (action.label === 'Edit' && hasDefaultEdit) return;
    ...
    updateSortIcons() {
        ...
        this.tableContainer.find('th[data-sort] i.sort-icon')
            .removeClass('fa-sort-up fa-sort-down text-black')
            .addClass('fa-sort text-gray-500');
    ...
    toggleFullscreen() {
        ...
        if (!document.fullscreenElement) {
            ...
            if (promise) {
                promise.catch(err => console.error(`Fullscreen request error: ${err.message} (${err.name})`));

                this.container.addClass('is-fullscreen');

                this.toolbarContainer.find(`#fullscreen-${this.options.containerId} i`).removeClass('fa-expand').addClass('fa-compress');
            }
        } else {
            ...
            if (promise) {
                promise.catch(err => console.error(`Exit Fullscreen error: ${err.message} (${err.name})`))
                    .finally(() => {
                        this.container.removeClass('is-fullscreen');
                        this.toolbarContainer.find(`#fullscreen-${this.options.containerId} i`).removeClass('fa-compress').addClass('fa-expand');
                    });
            } else {
                this.container.removeClass('is-fullscreen');
                this.toolbarContainer.find(`#fullscreen-${this.options.containerId} i`).removeClass('fa-compress').addClass('fa-expand');
        ...
        $(document).off(`fullscreenchange.${this.options.containerId} webkitfullscreenchange.${this.options.containerId} msfullscreenchange.${this.options.containerId}`)
            .on(`fullscreenchange.${this.options.containerId} webkitfullscreenchange.${this.options.containerId} msfullscreenchange.${this.options.containerId}`, () => {
               ...
               <div class="relative p-4 w-full max-w-md max-h-full">
                   <div class="relative bg-white rounded-lg shadow-xl border border-gray-300">
                       ...
                       <div class="p-4 md:p-5 text-center">
                           ...
                           <button id="confirm-delete-${this.deleteModalId}" type="button" class="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center me-2" data-action-method="" data-action-route="">
                               Yes, I'm sure
                           </button>
                           <button type="button" class="modal-close-button text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10" data-modal-hide="${this.deleteModalId}">No, cancel</button>
                       </div>
                   </div>
               </div>
           </div>
                 ...
                 <div class="relative bg-white rounded-lg shadow-xl border border-gray-300">
                     <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                      ...
                      <div class="p-4 md:p-5 space-y-4">
                          ...
                          <div class="edit-modal-error text-red-600 text-sm mt-2"></div>
                     </div>
                     <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b">
                         ...
                         <button type="button" class="modal-close-button ms-3 text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10" data-modal-hide="${this.editModalId}">Cancel</button>
                     </div>
                 </div>
             </div>
         </div>
            ...
            $formFieldsContainer.append(`
                    <div class="col-span-1">
                        ...
                        <input type="text" id="${fieldId}" name="${column.field}" value="${escapedValue}"
                         ...
                         <div class="edit-field-error text-red-500 text-xs mt-1" data-field-error="${column.field}"></div>
                    </div>
              `);
        });
    }
